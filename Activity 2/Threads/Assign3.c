...
states[j].who = j;
if (pthread_create(&(states[j].t), pthread_attr_default, (void *)moo, &(states[j])) != 0)
{
...
/* Wait a while then exit: all existing threads will die */
   ...
   void moo(struct state *s)
 {
     ...
     float r;     
     /* random number in interval 0.0 .. 1.0 */
